...
function measureLayout() {
  ...
  return { topPadding: navbarHeight + extraTopGap };
}
...
d3.json("data.json").then(data => {
  ...
  treeLayout = d3.tree().nodeSize([verticalSpacing-10, horizontalSpacing+80]);
  if (root.children) root.children.forEach(collapse);
  root.x0 = layout.topPadding;
  root.y0 = 0;
  update(root);
});
...
d3.select("#searchInput").on("input", function () {
  ...
  function searchAndHighlight(node) {
    ...
    const match = name.toLowerCase() === searchTerm || (searchTerm.length > 2 && name.toLowerCase().includes(searchTerm));
    const allChildren = (node.children || []).concat(node._children || []);
    let childMatch = false;
    allChildren.forEach(c => { if (searchAndHighlight(c)) childMatch = true; });
    if (match || childMatch) { if (node._children) { node.children = node._children; node._children = null; } }
    else { if (node.children) { node._children = node.children; node.children = null; } }
    node.match = match;
    return match || childMatch;
  }

  root.children = root._children || root.children;
  root._children = null;
  if (searchTerm === "") { root.descendants().forEach(d => d.match = false); if (root.children) root.children.forEach(collapse); }
  else { searchAndHighlight(root); root.match = false; }
...
function update(source) {
  ...
  const maxY = d3.max(nodes, d => d.y + maxNodeWidth); // include node width

  svg.attr("width", maxY - minY + padding * 2)
     .attr("height", maxX - minX + padding * 2 + layout.topPadding + extraBottomPadding);

  g.attr("transform", `translate(${padding - minY}, ${padding - minX + layout.topPadding})`);
  ...
  nodeEnter.append("rect")
    .attr("width", d => Math.min(maxNodeWidth, Math.max(80, (d.data.name || "").length * 9)))
    .attr("height", 24)
    .attr("x", 0).attr("y", -12)
    .attr("rx", 6).attr("ry", 6)
  ...
  nodeUpdate.select("text")
    .style("fill", d => d.match ? "#ff6b6b" : (d.data.url ? "#2ecc71" : "#e6eef6"));

  nodeUpdate.on("mouseover", function (event, d) {
      d3.select(this).select("rect").style("fill", d.match ? "#980101ff" : (d.data.url ? "#00d7e7ff" : "#3aa4ff"));
      d3.select(this).select("text").style("fill", d.match ? "#ffffffff" : "#041826");
    })
    .on("mouseout", function (event, d) {
      d3.select(this).select("rect").style("fill", "#0f1720");
      d3.select(this).select("text").style("fill", d.match ? "#ff6b6b" : (d.data.url ? "#2ecc71" : "#e6eef6"));
    });
